...
//
// 데이터 로딩은 3단계로 동작한다(환경에 따라 자동 선택):
//   1) 서버 프록시  /api/kospi-night  (Node/Express·Vercel 배포 시)
//   2) 정적 호스팅(GitHub Pages 등)이면 CORS 프록시로 네이버 시세 직접 조회
//   3) 둘 다 막히면 데모(시뮬레이션) 데이터 — "데모" 배지로 명확히 표시
(function () {
  ...
  let timer = null;
  let directMode = forceDemo ? false : null; // null=미정, true=서버없음(직접/데모)
  ...
  function toTime(v) {
    ...
    if (d.length >= 8) {
      const t = Date.parse(`${d.slice(0,4)}-${d.slice(4,6)}-${d.slice(6,8)}T${d.slice(8,10)||'00'}:${d.slice(10,12)||'00'}:00+09:00`);
      if (Number.isFinite(t)) return t;
    }
    const t = Date.parse(s); return Number.isFinite(t) ? t : null;
  }

  // 한국시간 기준 야간선물 거래시간(평일 18:00~익일 05:00)
  ...
  function parseQuote(obj) {
    ...
    return {
      ...
      changeRate, prevClose, time: q.localTradedAt || q.tradeTime || q.time || new Date().toISOString(),
  ...
  function parseSeries(obj) {
    ...
    for (const it of arr) {
      let t, v;
      if (Array.isArray(it)) { t = toTime(it[0]); v = toNum(it[4] ?? it[1]); }
  ...
  function demoQuote() {
    ...
    return { status: 'ok', source: 'demo', name: '코스피200 야간선물 (데모)',
      value: +(base + change).toFixed(2), change, changeRate: +((change / base) * 100).toFixed(2),
      prevClose: base, time: new Date().toISOString(), fetchedAt: new Date().toISOString(), session: nightSession() };
  }
  function demoSeries(points = 120) {
    const base = 345.0, now = Date.now(), out = []; let v = base;
    for (let i = points - 1; i >= 0; i--) { v += (Math.random() - 0.5) * 0.9; out.push({ t: now - i * 60000, v: +v.toFixed(2) }); }
  ...
  async function fetchDirectQuote() {
    for (const u of NAVER_QUOTE_URLS) {
      const obj = await proxyJson(u);
      if (obj) { const q = parseQuote(obj); if (q) return { status: 'ok', source: '네이버(직접)', fetchedAt: new Date().toISOString(), session: nightSession(), ...q }; }
    }
    return null;
  }
  ...
  async function loadQuote() {
    ...
    if (directMode !== true) {
      ...
      if (d && d.status === 'unavailable') { directMode = false; /* 서버는 있으나 소스 실패 → 직접도 시도 */ }
      else if (d === null) directMode = true; // 서버 API 자체가 없음(정적 호스팅)
    }
    const direct = await fetchDirectQuote();
    if (direct) return render(direct);
    return render(demoQuote()); // 최종 폴백
  }
  ...
  async function loadHistory() {
    ...
    // 2) 직접
    for (const u of NAVER_CHART_URLS) {
      const obj = await proxyJson(u);
      if (obj) { const s = parseSeries(obj); if (s) return applySeries(s, '네이버(직접)'); }
    }
    // 3) 데모
